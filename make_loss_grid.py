...
#G = np.hstack((I, B))
Master = master(hill_train_x,G, 3)
...
for i in range(repetitions):
    for w_lvl in range(3, 8):
        for grd_lvl in range(1, 5):
            ...
            loss_grid[w_lvl - 3, grd_lvl - 1] += get_loss(w, hill_train_x, hill_train_y)
            iters_grid[w_lvl - 3, grd_lvl - 1] += num_iters
            test_loss[w_lvl - 3, grd_lvl - 1] += get_loss(w, hill_test_x, hill_test_y)
